                ...
                Target(pack, DependsOn(test), () =>
                {
                    ...
                    foreach (var project in projects)
                    {
                        ...
                        if (isBeta)
                        {
                            ...
                            Run("dotnet", $"pack {project} -c Release -o {artifactsPath} --no-build --version-suffix beta");
                        }
                        else if (isAlpha)
                        {
                            //do the alpha pack
                            //--version-suffix is only working when you don't use Version in your csproj but VersionPrefix
                            Run("dotnet", $"pack {project} -c Release -o {artifactsPath} --no-build --version-suffix alpha");
                        }
                        else
                        {
                            Run("dotnet", $"pack {project} -c Release -o {artifactsPath} --no-build");
                        }
                    }
                });
